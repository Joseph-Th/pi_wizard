...
Process wrappers are part of the ownership problem. On Windows, a discovered/configured `.cmd` or `.bat` Pi launcher, including the standard npm `pi.cmd`, is invoked through the system `cmd.exe` as an app-owned hidden wrapper using `/d /v:off /s /c`. The wrapper keeps Pi's public launcher contract intact instead of inferring npm package-internal Node/CLI paths. `CREATE_NO_WINDOW` prevents a console, and the captured wrapper PID is the root of the exact Windows process tree used for Stop/Close/shutdown escalation, so the delegated Node child remains under the same lifecycle owner. Unsupported script hosts such as unresolved PowerShell launchers fail before live spawn. On Unix the child is launched as leader of an app-owned process group and hard termination targets that group. Neither path scans for `pi`, `node`, or shell processes by name. If tree/group termination cannot be confirmed, the run is quarantined.
...
`agent_start` and `agent_settled` mutate agent activity, not process lifetime. `agent_settled` is also the semantic end of the model/tool turn: any active tool preview that did not receive its normal `tool_execution_end` display event is discarded so an idle run cannot retain stale “running tool” UI, and the run's change revision advances conservatively because that unfinished extension-defined tool may still have mutated the execution root. Direct RPC Bash is not cleared by this rule because it has an independent request lifecycle. Process exit/failure clears hot activity. UI route changes never mutate either state axis.
...
Execution-root exclusivity also applies inside one live run. Pi may acknowledge an idle `prompt` before it emits authoritative `agent_start`; `RuntimeManager` therefore retains a backend-only pending-agent-start owner from successful request write until `agent_start`, settlement, rejection/write failure, Stop, or terminal cleanup. A second idle Prompt or direct Bash cannot cross that handoff window. Direct Bash then owns the run's execution root for its independent request lifetime: Prompt/Steer/Follow-up, model/thinking/queue-mode changes, compaction, session replacement/fork/clone/name mutation, another Bash, and Close are rejected until Bash completes or is cancelled. Read-only Pi probes/session export and `abort_bash` remain available. The active direct-Bash projection is authoritative renderer state, so reload cannot make conflicting controls appear available merely because component-local request state was lost.

Continuous Supervision follows the same ownership rule. A run with active direct Bash is not idle-actionable and is described to the supervisor as `command_running`. If user Bash starts after an idle generation was already selected for a supervisor decision, every autonomous directive for that run is skipped while Bash owns the root; a decision-required generation is removed from the considered-idle set so Bash completion's semantic wake causes it to be reconsidered rather than consumed or duplicated.
...
Wide Dashboard and selected-Run surfaces may mount a right-side context rail. Run health/activity, queue depth, tool/dialog counts, elapsed time, model, and thinking level come only from the existing bounded hydration projection. Session token/context/cost/message/tool-call metrics come from Pi's explicit `get_session_stats` RPC and are pulled when a selected session becomes ready, after a working turn settles, or by explicit refresh. There is no interval sampler and no app-owned provider-account budget estimate; provider quota/balance state is shown only if a future authoritative Pi/provider surface exists.
...
One Prompt-chain execution is an event-driven state machine layered over the existing `RuntimeManager`. Admission always goes through the manager's live-run ceiling. The chain is intrinsically sequential: every prompt starts a new ordinary Pi process/session in the selected project's local checkout with the model/thinking selection captured when the chain starts. The public request and execution snapshot expose no worker-pool or worktree-isolation switches.

Pi's successful `prompt` response is only acceptance; it is not evidence that the model task succeeded. After submission, the scheduler waits for a new Pi `agent_settled` generation and for the run to be genuinely idle with no queue, retry/compaction activity, or pending extension dialog. A separate backend-only assistant-message generation advances only for assistant `message_end`; `toolResult` `message_end` events do not advance it. The settled step must have produced a new assistant message whose final stop reason is `stop`. `error`, `length`, `aborted`, an unknown/missing outcome, or settlement whose latest assistant message is still `toolUse` are recorded as step failures. Provider error text from `errorMessage` is retained as the step error. This matches Pi's distinction between message boundaries and the session-level `agent_settled` boundary and prevents a tool loop, retry, or queued continuation from being mistaken for completion.

Prompt templates and skills remain valid chain prompts because Pi expands them into ordinary prompt input. A slash command that Pi reports as an extension command is rejected before submission: Pi documents extension commands as immediate handlers that manage their own LLM interaction, so they do not provide the ordinary model-turn settlement contract a Prompt-chain step requires. Command discovery failure after the worker starts is handled as a failed start and closes that exact worker rather than leaking a live process. After every settled success or failure, the automation owner closes the fresh Pi process to release its live slot and advances to later prompts. Completion detection never probes `get_session_stats`.

The runtime manager broadcasts a cheap semantic run-state wake-up in addition to the renderer's coalesced dirty wake-up. Prompt chains consume only semantic wake-ups, including successful live-capacity changes, so token/tool display traffic cannot drive scheduler work and renderer draining is not required for a chain to progress. Launches and live-limit mutations share the desktop serialization gate; admission and local execution-root ownership are rechecked under that gate. Cancellation is rechecked before process spawn so a cancelled not-yet-started step cannot race into a new Pi process.

The complete Prompt-chain scheduler is covered by deterministic desktop integration, not only helper contracts. The fixture launches a distinct real child RPC process/session for every step and covers a normal assistant/tool/tool-result/final-assistant loop, settlement whose final assistant outcome is only `toolUse`, an accepted prompt that later ends with a provider `error`, a recognized immediate extension command, a prompt rejected before acceptance, and later successful work. Failures remain attached to their exact step and do not prevent later prompts from being attempted.

Automation renderer projection is also separated by cost. Saved chain definitions are hydrated only while the Automation view is mounted. Catalog changes invalidate the full view model, while execution changes fetch only bounded execution snapshots. Identical execution mutations emit no invalidation, and each step snapshot carries only a UTF-8-safe prompt preview (1 KiB by default) rather than repeatedly cloning full saved prompt text across IPC. During the one-time global-to-project migration, an older same-name legacy chain is not imported when the project-local catalog already contains that legacy prompt sequence as an exact trimmed prefix of a longer local chain; the richer local definition remains authoritative and the redundant legacy identity is discarded only as part of the successful destination-first migration.
...
Starting supervision selects one or more present registered projects plus an explicit supervisor model/thinking choice. One selected Git-backed project hosts the supervisor's generated worktree; the selected project set remains the target authority and may include other projects or repository subdirectories. The supervisor is itself a normal Pi run and therefore consumes one live-run slot. It launches with project context files/extensions disabled so supervision is not coupled to arbitrary project extension UI. Its eligible targets are other nonterminal live runs whose `ProjectId` is in the selected set, regardless of whether they were started manually or by Automation. The supervisor never targets its own RunId. Overlapping active supervision project sets are rejected.

Production Supervision has no finite-cycle or Prompt-chain-playbook request mode. The coordinator is event-driven and continuous until explicit Stop or failure. It subscribes to the same semantic run-state wake-up used by Prompt chains and keeps a bounded per-target marker containing four backend observations: session-replacement generation, current Pi session identity, `agent_settled` generation, and assistant-message generation. Session replacement advances its generation immediately when Pi accepts `new_session`, `switch_session`, `fork`, or `clone`, before the follow-up `get_state` establishes the replacement session ID. An eligible run is considered once if already idle when supervision starts and again after each new settled generation. `message_end` alone does not wake a new decision, and a `toolResult` message is never an assistant result. These backend-only observations remove any need for `get_session_stats` polling and make a deliberate no-op or unrelated semantic wake non-reentrant.

The desktop sends bounded project/run/status plus the recorded assistant outcome/error and, when Pi has one, a bounded native `get_last_assistant_text` excerpt. Pi explicitly permits that RPC to return `text: null`; absence of text therefore does not erase an `error`, `aborted`, `length`, or other recorded outcome. After the supervisor's own Pi turn reaches a new `agent_settled`, the desktop requires a fresh assistant message with final stop reason `stop` and non-null directive text before parsing one narrow JSON object. A supervisor provider error, abort, length stop, tool-use-only settlement, unknown outcome, or missing text fails the supervision owner rather than reusing stale output.

Supported directives are `send`, `steer`, `follow_up`, and `stop`. Each directive is bounded, duplicate targets in one cycle are rejected, and target membership/lifecycle plus the observed session/settlement/result version are freshly revalidated immediately before mutation. Any directive is skipped if the target crossed a Pi session-replacement boundary or now reports a different session ID; `send` and `stop` additionally require the same settled/result generations that triggered the decision, so newer manually completed work cannot receive an older autonomous continuation. Send otherwise requires an idle target; steer/follow-up require active work. A directive that has become stale because the target finished, changed activity, switched sessions, or completed newer work is skipped and recorded rather than terminating the long-lived supervisor; unknown/ineligible targets and malformed autonomous output still fail closed. Stop uses the runtime manager's exact owned-process lifecycle path and fails on termination uncertainty instead of claiming the worker stopped. After validated directives are applied, the coordinator replaces a single bounded `last_decision` display summary; it does not retain raw supervisor response history or create another transcript authority.

Every supervisor model turn has a deadline. A private finite-cycle ceiling exists only in deterministic service tests; public IPC and renderer state expose continuous Supervision only. `cycles` counts completed supervisor decisions, not model turns cancelled before application. Explicit user Stop is checked before supervisor submission, after its model turn, and between directives, so pending future autonomous actions are abandoned; a directive already being written at the instant Stop arrives is governed by the runtime request boundary and cannot be retroactively unsent. A Stop requested before supervisor spawn yields `Stopped`, not `Failed`; once a supervisor child exists, its exact RunId is recorded before the readiness wait, allowing Stop during the startup handshake to terminate that owned Starting process. Invalid autonomous output or internal failure ends the supervision owner and performs bounded termination of its app-owned supervisor process. User Stop does not terminate observed workers; only an explicit validated supervisor `stop` directive targets a worker. An abrupt desktop exit is handled by the Windows process-containment boundary described below.

### Pi-native run utilities

Run details expose narrow Pi operations without creating alternate runtime owners. Session HTML export sends Pi's native `export_html` RPC and accepts only the bounded returned path. The one-shot command control sends Pi's direct `bash` RPC with `excludeFromContext=true` against the run's immutable execution root; streamed `bash_execution_update` data uses the existing request-correlated bounded live projection, the final response is truncated again to the tool-preview byte ceiling before renderer retention, and `abort_bash` is the cancellation path. Direct Bash is an execution-root writer even though the normal agent activity axis remains Idle, so the runtime excludes it from model prompts, steer/follow-up, compaction/model/session mutations, session replacement, and Close for the same run. The reciprocal prompt path owns the narrow interval after an idle `prompt` has been written but before Pi emits authoritative `agent_start`; a second prompt or direct Bash cannot enter that gap. Read-only probes/export and direct-Bash cancellation remain available. A completed direct Bash request invalidates Git review through the existing change-revision mechanism regardless of exit status. No PTY, shell history, persistent terminal process, or renderer-spawned command path is introduced.
...
Pi Wizard treats the public Pi launcher as the compatibility boundary. When Windows discovery resolves the extensionless npm shim or a `.cmd`/`.bat` Pi launcher, the environment owner selects the sibling command shim when necessary and invokes it as `cmd.exe /d /v:off /s /c <pi-shim> ...`. The process owner starts that wrapper with private piped stdio and `CREATE_NO_WINDOW`; Pi's delegated Node process inherits those pipes and remains a descendant of the captured wrapper PID. The runtime does not inspect `node_modules`, derive `@earendil-works/pi-coding-agent/dist/bundle/cli.js`, or require Node to live in a particular directory. Unsupported script hosts fail explicitly before live spawn.
...
Pi Wizard's hot `get_entries` projection has stricter entry/byte ceilings than the transport frame. A successful Pi page that exceeds those local projection ceilings is likewise a classified resync condition: the controller releases the correlated request normally, marks live synchronization as requiring resync, emits the session-sync revision, and leaves the Pi process Ready. The selected history owner can then reload the bounded latest JSONL page and reseed the append cursor. Malformed entry/cursor shapes remain protocol errors; only the app-local page count/byte limits take this recoverable path.

The latest cold-history page is the bootstrap boundary for that live synchronization. Pi may advertise the future JSONL path of a new persistent session before its first entry creates that file. For the newest page only, an advertised missing file is therefore a valid empty-session state only while the same authoritative session reports `messageCount == 0` and live synchronization has not already observed a persisted entry cursor; a missing file after a cursor has been observed, after a nonzero message count, or during older-page navigation remains a hard history error. This extra cursor guard matters because `get_state.messageCount` may remain stale after a completed turn. The empty newest page seeds live synchronization at a null cursor and immediately issues `get_entries(null)`, so the first later settlement can advance to the first persisted entry without requiring another `get_state`. Persisted pages still require exact RunId, session ID, and canonical session-path revalidation before their append cursor/leaf seeds the manager. If that retained run is already terminal, the JSONL page is returned without attempting `get_entries`: there is no live append race left to close, and persisted history must remain readable after the owned Pi process exits. Older history pages never rewind the live cursor. The renderer records the session-sync revision observed at the start of its latest-page read and treats a later revision increase as new persisted activity independently of `get_state.messageCount`. This closes both the first-entry transition and the normal settlement race where Pi has appended the final answer and emitted `agent_settled` but no new `get_state` reconciliation has occurred: the lower transient answer may disappear at semantic settlement only because the upper persisted conversation follows the session-sync revision and reloads the bounded latest page.
...
The desktop uses one portable app-owned state root named `pi-wizard-data` for custom model identities, project registrations, worktree recovery records, preferences, and session drafts. A repository build whose executable is under `<repo>/target/debug` or `<repo>/target/release` resolves that root to `<repo>/pi-wizard-data`, outside disposable Cargo output; a standalone executable resolves it beside itself. Those domains therefore survive repository cleans/rebuilds without depending on Windows AppData. If the current root does not exist, startup migration prefers the legacy executable-sibling `pi-wizard-data` location and otherwise the legacy AppData `runtime-state`; an existing current root always wins so migration input cannot overwrite current local state.

Prompt-chain definitions are deliberately outside that global root. The selected registered project's canonical root owns exactly one bounded catalog at `<project>/.pi-wizard/prompt-chains.json`. Catalog reads do not create `.pi-wizard`; the directory is created only when a chain is first persisted or a corrupt local catalog must be quarantined. Save, delete, list, and start all require the same `ProjectId`, revalidate that registered project as present, and resolve the catalog from that exact canonical project root. A chain ID from one project cannot be looked up or started through another project, and there is no AppData or portable-state fallback for prompt-chain definitions. On upgrade from the former global `pi-wizard-data/automation-chains.json` store, startup migrates that legacy catalog only when exactly one registered project is presently resolvable; it commits the merged project-local catalog before deleting the legacy file and refuses to guess a destination when project ownership is ambiguous. Corrupt project-local catalogs are quarantined under the same project's `.pi-wizard/prompt-chain-quarantine` directory without affecting other projects.
...
- for the latest active-session window, follow either authoritative message-count growth or the independently monotonic live session-sync revision so a settled final answer cannot vanish while `get_state.messageCount` is stale;
