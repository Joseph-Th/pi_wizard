...
The latest cold-history page is the bootstrap boundary for that live synchronization. Only after the backend revalidates the exact RunId, session ID, and canonical session path does it seed the manager from the page's append cursor/leaf, then queue one incremental `get_entries(since)` catch-up. Older history pages never rewind the live cursor. The renderer records the session-sync revision observed at the start of its latest-page read and treats a later revision increase as new persisted activity independently of `get_state.messageCount`. This closes the normal settlement race where Pi has appended the final answer and emitted `agent_settled` but no new `get_state` reconciliation has occurred: the lower transient answer may disappear at semantic settlement only because the upper persisted conversation follows the session-sync revision and reloads the bounded latest page.
