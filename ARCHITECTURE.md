...
The user-facing Stop operation means stop all currently scheduled work for that run. Pi's `abort` does not clear queued steering/follow-up messages, so Stop first sends `clear_queue`, validates the returned queue against dedicated message/byte ceilings, preserves it as recoverable composer content, and then sends `abort`. A lower-level Abort-current-work action, if ever exposed, may intentionally leave queues intact, but it must use different language.
...
`get_state` is also an authoritative reconciliation source. On startup/recovery the backend can restore streaming/compaction activity, session ID/path/name, model, thinking level, steering/follow-up modes, auto-compaction state, and message counts from the child without replaying renderer history. Event deltas remain the normal live path; the state response repairs gaps.
...
Pi extension `set_editor_text` mutates this same backend draft before renderer notification. Hydration schema v6 exposes the current draft plus restore/submission state and immutable worktree identity so reload recovery is backend-owned rather than dependent on component state.
...
- Enumerate only enough metadata to populate the first visible page.
...
- File changes invalidate only the affected catalog entry.
- Migration work is versioned, bounded, cancelable, and never runs as an unbounded startup monopoly.

A SQLite/FTS implementation is permitted if measurements justify it, but the architecture depends on the `SessionCatalog` contract rather than SQLite itself.
...
- four concurrent simulated Pi runs with bursty events.

Exact fixture sizes can evolve from real Pi data, but the architecture must remain bounded as these totals grow.

## 15. Observability

Developer diagnostics should expose bounded counters instead of giant logs:
...
Diagnostics use ring buffers and size caps. Turning on tracing must not reproduce the disk/memory amplification the diagnostics are intended to find.
