...
The production UI starts supervision with no cycle ceiling. The coordinator is event-driven and continuous until explicit Stop or failure. It subscribes to the same semantic run-state wake-up used by Automation and keeps a bounded per-target marker containing three backend observations: a session-replacement generation, the current Pi session identity, and the assistant-message generation last considered while idle. The session-replacement generation advances immediately when Pi accepts `new_session`, `switch_session`, `fork`, or `clone`, before the follow-up `get_state` has established the replacement session ID; both generations are skipped from renderer serialization. An eligible run is actionable once if already idle at supervision start and once again after each later authoritative assistant `message_end` returns the run to an actionable idle state. Because these generations are derived inside the runtime event/response path, Supervision does not issue `get_session_stats` probes to discover whether workers or the supervisor itself have completed turns. Recording the considered version before asking the supervisor prevents a deliberate no-op or an unrelated semantic wake from becoming a busy loop, while still detecting a fast Working-to-Idle turn that occurs between coordinator observations. Token/tool display traffic never triggers supervision.

The desktop sends bounded project/run/status plus native `get_last_assistant_text` excerpts and an optional bounded list of saved Automation prompts as a playbook. The playbook is advisory input, not an execution queue. After the supervisor's normal Pi turn settles, the desktop parses one narrow JSON object containing directives addressed to exact worker RunIds. Supported directives are `send`, `steer`, `follow_up`, and `stop`. Each directive is bounded, duplicate targets in one cycle are rejected, and target membership/lifecycle plus the observed session version are freshly revalidated immediately before mutation. Any directive is skipped if the target crossed a Pi session-replacement boundary or now reports a different session ID; `send` and `stop` additionally require the same assistant-message generation that triggered the decision, so a newer manual result that has already returned Idle cannot receive an older autonomous continuation. Send otherwise requires an idle target; steer/follow-up require active work. A directive that was valid for the observed run but has become stale because the target finished, changed activity, switched sessions, or produced newer work before application is skipped and recorded rather than terminating the long-lived supervisor; unknown/ineligible targets and malformed autonomous output still fail closed. Stop uses the runtime manager's exact owned-process lifecycle path and fails on termination uncertainty instead of claiming the worker stopped. After validated directives are applied, the coordinator replaces a single bounded `last_decision` display summary describing the applied or skipped action(s); it does not retain raw supervisor response history or create another transcript authority.

Runtime limits still cap an optional finite cycle budget for deterministic tests or future bounded callers, and every supervisor model turn has a deadline. `cycles` counts completed supervisor decisions, not model turns that were cancelled before application. A finite owner terminates immediately after applying its final permitted decision rather than waiting for another worker result to notice the budget is exhausted. Explicit user Stop is checked before supervisor submission, after its model turn, and between directives, so pending future autonomous actions are abandoned; a directive already being written at the instant Stop arrives is governed by the runtime request boundary and cannot be retroactively unsent. A Stop requested before supervisor spawn yields `Stopped`, not `Failed`; once a supervisor child exists, its exact RunId is recorded before the readiness wait, allowing Stop during the startup handshake to terminate that owned Starting process. Invalid autonomous output or internal failure ends the supervision owner and performs bounded termination of its app-owned supervisor process. User Stop does not terminate observed workers; only an explicit validated supervisor `stop` directive targets a worker. An abrupt desktop exit is handled by the Windows process-containment boundary described below.
