...
- **Windows npm launchers:** standard npm Pi exposes a public `pi.cmd` launcher that delegates to its installed Node entry point. Pi Wizard treats that shim as the compatibility boundary and runs it through a hidden, exact-tree-owned Windows command wrapper instead of deriving a package-internal `node_modules` path. Repository fixtures and the optional packaged real-Pi smoke own this compatibility shape.
