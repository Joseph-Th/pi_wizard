...
On standard Windows npm Pi installs, Pi Wizard resolves the npm shim to Node plus Pi's CLI entry point and launches that directly with no console window instead of keeping a command-shell wrapper alive. Graceful RuntimeManager shutdown still owns normal lifecycle. A desktop-lifetime Windows kill-on-close Job Object provides the abrupt-exit backstop so inherited Pi/Node descendants cannot be orphaned if the GUI process terminates unexpectedly.
