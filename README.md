...
Pi Wizard keeps its own durable state in a portable `pi-wizard-data` directory beside `pi-wizard-desktop.exe`, not in Windows AppData. That directory contains prompt chains, custom model profiles, project registrations, worktree recovery records, preferences, and session drafts through their existing bounded atomic stores. Rebuilding or replacing the executable in the same directory keeps that data. Custom models are added/removed at runtime from the model picker and are persisted in `model-profiles.json`; adding one does not require recompiling Pi Wizard. A one-time upgrade migration imports the older AppData `runtime-state` tree only when no portable state directory already exists.
