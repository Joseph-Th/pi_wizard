...
It launches Pi only as `--mode rpc --no-session --no-context-files --no-extensions --no-approve --offline`, sends `get_state`, `get_available_models`, `get_available_thinking_levels`, `set_auto_retry(false)`, and a harmless empty `clear_queue` capability probe, bounds captured output/deadlines, verifies that no session file was created, then exits on stdin EOF. It never sends `prompt`, so it does not create a provider model request. `clear_queue` success is reported as supported; an exact unknown-command response is reported as unavailable rather than failing the smoke because current stable Pi 0.84.3 has this documented/runtime skew. Other `clear_queue` rejections still fail. Optional `--provider`, `--model`, and `--thinking` arguments can verify explicit launch selection without prompting the model.
...
Automation tests additionally prove that saved chains round-trip through atomic bounded persistence, malformed/unsupported chain state quarantines independently, failed validation never mutates the existing catalog, and execution snapshots retain only bounded chain-step metadata plus UTF-8-safe prompt previews. Desktop contracts distinguish catalog from execution invalidation, suppress identical execution updates, keep catalog hydration off ordinary startup/runtime refresh paths, preserve working workers when a chain is cancelled, and use the full compact execution UUID in generated worktree branch/path identity so two UUIDv7 executions sharing the same timestamp prefix cannot collide. The scheduler consumes the runtime manager's semantic state-change channel rather than renderer dirty/token traffic, and a live-limit change wakes capacity-blocked orchestration; the full eight-run fixture establishes the shared concurrency ceiling. Manual and automated starts remain subject to the same backend admission, launch serialization, and exact execution-root ownership. Supervisor admission tests prove it cannot consume the only slot required for first-worker progress and that an already-running internal supervisor yields under later full-capacity manual-run pressure when queued worker work has no active worker. Worker completion tests cover both new-assistant-message detection and observed-real-activity fallback for user Stop/abort turns. Supervisor tests must keep the supervisor inside the ordinary Pi runtime, count it against live capacity, expose only bounded task/status/last-result summaries, reject unknown/duplicate/invalid directives, enforce bounded cycle/turn lifetime, and leave the deterministic chain operational if autonomous supervision is disabled. Scheduler-failure handling must preserve running worker sessions while attempting to terminate the internal supervisor and marking never-started steps explicitly failed. Desktop integration tests also exercise the complete scheduler against real child-process RPC and disposable real Git repositories: a sequential local chain must launch one distinct Pi child per prompt, continue after an isolated prompt rejection, and release all live capacity; a four-step worktree chain must actually reach two simultaneous workers and retain four unique verified worktrees; generated worktree-parent creation must occur before the first Git mutation, while a proven non-mutating creation failure discards its recovery intent; and a supervised chain must launch a separate supervisor child/worktree, accept one strict JSON `send` directive, revalidate its RunId, route that second prompt into the exact existing worker session, and release both process slots at completion.
...
- successful completed persisted tool/Bash output is collapsed by default but explicitly expandable, failed output remains expanded, and verbose completed thinking collapses while active/short thinking remains visible;
- hydration snapshots with any schema other than the renderer's supported v9 contract are rejected before application instead of partially applying incompatible state;
- Run details, Changes, and Session Tree are mutually exclusive, closed-by-default inspector mounts; first opening Changes triggers the bounded summary request, switching away cancels in-flight review work, and Session Tree cannot retain a late result after unmount;
