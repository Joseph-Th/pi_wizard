...
- a successful `get_entries` response whose projected page exceeds the configured entry/byte ceiling requests explicit cold-history resynchronization while the process stays Ready; malformed entry/cursor shapes still fail rather than being downgraded to a local-size recovery;
...
Prompt-chain service tests additionally prove that saved chains round-trip through atomic bounded persistence, malformed/unsupported chain state quarantines independently, failed validation never mutates the existing catalog, and execution snapshots retain only bounded chain-step metadata plus UTF-8-safe prompt previews. Global-to-project migration also proves that a same-name legacy chain is skipped when its trimmed prompt sequence is already a prefix of a richer project-local chain, preventing a stale global copy from creating an indistinguishable duplicate. The public start request and execution projection contain no concurrency/worktree mode: chains are sequential local-checkout workflows by construction, while the editor retains its chain/project/model draft across view navigation. A fresh desktop process must hydrate the first saved chain from the selected project's persisted catalog into the editor without requiring a click; selecting a saved chain must populate the editor, expose the selected state, and move focus into that editor. An explicit blank New chain must still survive ordinary view navigation without being replaced by automatic catalog hydration. Prompt chains use the same durable model preference as New Run rather than resetting to a separate model default when the project changes. Execution state contains no supervisor lifecycle fields. Desktop contracts distinguish catalog from execution invalidation, suppress identical execution updates, keep catalog hydration off ordinary startup/runtime refresh paths, and preserve a running step when a chain is cancelled.

The scheduler consumes semantic runtime wakeups rather than renderer dirty/token traffic and never polls `get_session_stats` for completion. A queued chain blocked by live-run capacity or another live run on the same local execution root must remain Running/Queued and wake after authoritative capacity/root release rather than terminating early. Protocol/integration fixtures prove that assistant `message_end` and `agent_settled` are separate generations; a `toolResult` `message_end` cannot count as an assistant result; a normal assistant-tool-toolResult-final-assistant loop completes only after the final `stop` and settlement; a settlement whose latest assistant result is `toolUse` fails; an accepted prompt that later produces assistant `stopReason: error` preserves its provider `errorMessage`; a recognized immediate extension slash command is rejected without leaking its fresh worker process; pre-acceptance prompt rejection is isolated; and later prompts still run. One integration execution also asserts a distinct Pi process/session identity for every attempted prompt.

Supervision tests are separate from Prompt chains. They prove the supervisor has its own coordinator/event/IPC state, is counted as one ordinary live Pi run, accepts a bounded set of registered `ProjectId` targets, rejects overlapping active supervision sets, and reacts only to semantic idle/settled-result transitions. Public production IPC has neither a Prompt-chain playbook nor a finite-cycle switch and probes model/thinking options against a selected project through the same `ModelPicker` used elsewhere. An already-idle run is considered once when supervision starts; each later `(session-replacement generation, session identity, agent-settled generation, assistant-message generation)` version is considered once. A deliberate no-op and unrelated runtime wake cannot spin without newer settled work. Provider-error fixtures prove `get_last_assistant_text: null` is valid and the recorded stop reason/error still reaches the supervisor context. The supervisor's own model turn must itself settle with a fresh final `stop` response before directive JSON is accepted. Direct Bash removes an otherwise-idle run from actionability; orchestration does not poll `get_session_stats`; stale directives are freshly revalidated before Send/Steer/Follow-up/Stop; user Stop abandons pending directives; and startup Stop targets the exact supervisor child. Private finite-cycle limits are test scaffolding only.

Custom-model catalog tests prove bounded provider/model/name validation, atomic round-trip, duplicate identity replacement, corruption quarantine, aggregate-byte limits, and merging where Pi-discovered metadata wins over a matching custom identity. Model-preference tests separately prove that a fresh preference store defaults New Run to `opencode-go/muse-spark-1.2-contributor`, an explicit Pi-default selection persists as null, the last explicit New Run model and bounded favorite identities survive reopen, schema-1 live-run preferences migrate to schema 2 without losing their run ceiling, and failed atomic writes do not mutate in-memory preference state. A desktop child-process regression probes multiple fake Pi models through the dedicated model-catalog operation without depending on thinking-level or Stop-compatibility probing. Renderer contracts prove the model picker is reusable by New Run, Prompt chains, and Supervision, loads Pi-discovered choices globally, retains Pi default as an explicit option, groups available favorites ahead of the ordinary model list, and makes New Run and Prompt chains share the durable model preference without clearing it on project/trust/context changes. The picker also allows custom provider/model entries without storing credentials. Model-catalog discovery is a separate IPC/probe from secondary launch-option validation so those secondary failures cannot erase an already valid Pi model list.
...
- standard Windows npm Pi installations resolve the public `pi.cmd`/`.bat` launcher into an app-owned hidden `cmd.exe /d /v:off /s /c` wrapper without inspecting package-internal Node/CLI paths; wrapper paths containing spaces preserve RPC stdio, `CREATE_NO_WINDOW` prevents a console, and exact-tree termination owns the delegated child process;
...
Prompt-chain persistence fixtures cover project ownership separately from the global app-state root: saving in project A creates exactly `A/.pi-wizard/prompt-chains.json`, opening project B does not expose A's chains or create B's directory on a read-only lookup, save/delete/start requests carry the selected `ProjectId`, and normal desktop startup does not open a prompt-chain catalog from portable/AppData state. Upgrade coverage separately proves the former `pi-wizard-data/automation-chains.json` catalog migrates only when exactly one registered project is presently resolvable, writes the project-local catalog before removing the legacy source, preserves conflicting local data, and does not guess among multiple projects.

Launch/trust tests cover Pi saved/default trust inheritance, one-run approve/ignore overrides, and the fact that ignoring protected project resources does not implicitly add `--no-context-files`. Extension discovery is an independent launch policy with an explicit one-run disabled path for local/worktree/recovered-worktree/resumed-session launches, and the launch-options probe itself is extension-free so a broken installed extension cannot block access to recovery settings. The desktop launch contract also proves provider/model must be supplied as an exact pair, startup model/thinking/context-file selections are applied before Pi spawn for local/worktree/recovered-worktree runs, resumed sessions preserve explicit context-file/extension policies, and the launch-options probe uses an ephemeral Pi RPC process rather than creating a saved session. That same already-owned probe sends `clear_queue`: accepted bounded empty output means reusable Stop is available; an unknown-command rejection becomes `clearQueueSupported: false` so the launcher warns about exact-process Stop fallback without adding another startup/background child. Custom session directories are canonicalized before spawn. A separate optional live smoke may verify the installed Pi's current model/thinking discovery, extension-free/no-session launch flags, native `set_auto_retry`, and `clear_queue` support state without making live Pi/provider prompts part of routine deterministic verification. After a release build, `PI_WIZARD_SMOKE_REAL_PI=1` runs the packaged desktop against the actually installed Pi and requires wrapper resolution plus a real Ready startup; adding `PI_WIZARD_REAL_PI_PROMPT` also requires one real model turn to enter Working, settle, and leave the process Ready. Routine `full` verification keeps this provider-backed smoke optional so deterministic release validation does not depend on credentials or network availability.
...
- Recent Sessions reuses the same bounded catalog/resume component as New Run, automatically loads the newest bounded page when the view mounts or its selected project changes, performs no periodic catalog work, leaves New Run's embedded browser on-demand, and invalidates an in-flight page response when the selected project changes;
...
- wide Dashboard/Run layouts expose semantic status tones with textual labels and aggregate live-run activity; dashboard cards pull lightweight Pi context/token facts on session change or turn settlement, while the selected-run rail exposes detailed `get_session_stats` metrics; neither path uses an interval sampler;
...
- live assistant/reasoning/direct-Bash presentation consumes only backend-bounded snapshots; rapid active-tool snapshots are summarized in the stable activity status rather than mounted as transient cards, reasoning remains continuous across Pi thinking/tool/thinking cycles while active, and settled assistant/reasoning blocks are no longer rendered in the lower pane once the authoritative agent-working state becomes false;
...
Desktop portable-state tests prove repository builds resolve `pi-wizard-data` outside `target`, standalone executables use an executable-sibling root, legacy `target\debug|release\pi-wizard-data` is preferred over legacy AppData during migration, nested legacy AppData state migrates intact when needed, and an existing current portable root is never overwritten by migration input. The shared root is consumed by the custom-model, project/worktree, preference, and draft stores. Prompt chains are excluded from that contract because their durable owner is the selected project's `.pi-wizard/prompt-chains.json` file.
