...
- a latest cold-history page bootstraps live `get_entries` only after exact run/session/path revalidation, and a settled-turn regression proves the live session-sync revision advances even when no new `get_state` arrives and `messageCount` remains stale;
