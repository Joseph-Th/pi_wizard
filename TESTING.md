...
- a packaged-WebView smoke that launches that exact release executable under a loopback-only WebView2 DevTools port, exercises representative custom IPC plus `plugin:event|listen`/`unlisten`, navigates every primary surface, and fails on visible ACL/CSP/runtime-update errors. It then launches a byte-identical disposable copy beside a deliberately empty portable state root to prove first-use Muse selection, New Run selection persistence across a real desktop restart, and favorites-first grouping without reading or mutating the user's real `pi-wizard-data`. This is the release boundary that catches Tauri capability and preference/UI integration mistakes which source-only or process-alive checks cannot detect.
...
- while a tool is active, at most one compact human-readable activity status explains the wait; raw tool/Bash protocol output is not rendered as conversation content;
...
- live assistant/reasoning presentation consumes only backend-bounded snapshots; reasoning remains prominent across Pi thinking/tool/thinking cycles while raw tool/Bash protocol output stays out of the transcript;
- persisted history preserves assistant reasoning separately from the answer and omits tool-only/Bash protocol messages entirely, so reopening a session cannot reintroduce protocol-noise rows;
