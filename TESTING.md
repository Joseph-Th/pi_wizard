...
Prompt-chain persistence fixtures cover project ownership separately from the global app-state root: saving in project A creates exactly `A/.pi-wizard/prompt-chains.json`, opening project B does not expose A's chains or create B's directory on a read-only lookup, save/delete/start requests carry the selected `ProjectId`, and desktop startup does not open a prompt-chain catalog from portable/AppData state.
