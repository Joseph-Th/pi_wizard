...
- wide Dashboard/Run layouts expose semantic status tones with textual labels, aggregate live-run activity, and selected-run Pi `get_session_stats` metrics; source contracts forbid an interval sampler and packaged-WebView coverage requires deterministic token/context/run facts after a settled turn;
