...
Automation-service tests additionally prove that saved chains round-trip through atomic bounded persistence, malformed/unsupported chain state quarantines independently, failed validation never mutates the existing catalog, and execution snapshots retain only bounded chain-step metadata plus UTF-8-safe prompt previews. The backend scheduler retains its bounded concurrency/worktree contracts for service coverage, but the product Prompt chains renderer must submit `concurrency: 1` and `worktrees: false`, retain its chain/project/model draft across view navigation, and expose no worker-pool or Git-isolation controls. Automation execution state contains no supervisor lifecycle fields. Desktop contracts distinguish catalog from execution invalidation, suppress identical execution updates, keep catalog hydration off ordinary startup/runtime refresh paths, and preserve a running step when a chain is cancelled. The scheduler consumes the runtime manager's semantic state-change channel rather than renderer dirty/token traffic. Completion tests cover both the backend-only assistant `message_end` generation and observed-real-activity fallback for user Stop/abort turns; completion detection must not require `get_session_stats` probe traffic. Desktop integration exercises both the user-facing sequential local chain and the backend's bounded worktree/concurrency paths against real child-process RPC and disposable real Git repositories.

Supervision tests are separate from Prompt chains. They prove the supervisor has its own coordinator/event/IPC state, is counted as one ordinary live Pi run, accepts a bounded set of registered `ProjectId` targets, rejects overlapping active supervision sets, and reacts only to semantic idle/result transitions. The production renderer supplies no Prompt-chain playbook (`promptTemplates: []`) and probes model/thinking options against a selected project through the same `ModelPicker` used elsewhere. An already-idle run is considered once when supervision starts; each later `(session-replacement generation, session identity, assistant message_end generation)` is considered once when the run returns idle; a deliberate no-op and unrelated runtime wake cannot spin without another authoritative assistant result. Direct Bash removes an otherwise-idle run from actionability, and orchestration does not use `get_session_stats` polling. The session-replacement generation is backend-only/serde-skipped and invalidates stale decisions across session replacement. Directives are freshly revalidated before Send/Steer/Follow-up/Stop, user Stop abandons pending directives, startup Stop targets the exact supervisor child, and the production request remains continuous (`maxCycles: null`).
