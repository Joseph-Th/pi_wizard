...
| Sessions | Pi JSONL remains authoritative. Cold/resumed history is file-backed and paged; the validated latest page seeds bounded `get_entries(since)` synchronization, whose monotonic revision lets the persisted conversation follow a settled final answer even before a later `get_state` updates message counts. Session search/resume, naming, tree inspection, fork/clone, usage, HTML export, compaction, and retry controls reuse Pi operations. |
