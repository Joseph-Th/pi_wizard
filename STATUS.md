...
| User data | Session drafts, project/worktree registries, prompt chains, custom model identities, and preferences persist under the portable `pi-wizard-data` root through bounded schema-versioned stores. Drafts are session-scoped and generation-safe. |
...
| Prompt chains | Finite saved prompt chains run one prompt at a time in the selected project's local checkout. Every prompt launches a fresh ordinary Pi session, waits for authoritative turn settlement, closes that process to release capacity, then advances. The view retains the current chain/project/model selection while navigating elsewhere. Cancellation stops future launches without terminating the currently running prompt. |
| Supervision | Supervision is independent from Prompt chains. One ordinary Pi supervisor run consumes one live slot and may cover multiple selected projects continuously. Newly idle results are considered once per backend session-replacement/session/message-end version without `get_session_stats` polling. Autonomous directives are revalidated against that exact observed version, so session switches and newer manually completed results make stale Send/Stop decisions non-actionable. Active user direct Bash temporarily removes a run from idle actionability; if Bash starts during a decision, all autonomous directives are deferred for that decision-required generation and Bash completion wakes reconsideration. User Stop abandons pending future directives, cancelled supervisor turns are not counted as decisions, and Stop before/during supervisor startup is a normal exact-process `Stopped` path. |
